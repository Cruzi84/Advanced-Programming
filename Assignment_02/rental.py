...
class Renter:
    ...
    @license_no.setter
    def license_no(self, number):
        if number <= 0:
            raise ValueError("License number must be positive")
        self._license_no = number
...
class ElectricCar(Vehicle):
    ...
    def __str__(self):
        status = "rented" if self.is_rented else "available"
        return f"{self.make} {self.model} ({self.plate}) [{status}] - {self.battery_kwh} kWh"
...
class Motorbike(Vehicle):
    ...
    def __str__(self):
        status = "rented" if self.is_rented else "available"
        return f"{self.make} {self.model} ({self.plate}) [{status}] - {self.engine_cc} cc"